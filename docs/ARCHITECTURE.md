...
- `open-computer-use call <tool> --args '{...}'` 会直接输出 MCP-style JSON result；`open-computer-use call --calls '[...]'` 会在同一进程里顺序执行 JSON 数组里的 tool calls，并复用同一个 `ComputerUseService` 内存态，因此 `get_app_state` 之后的 action tool 可以继续使用同一轮 snapshot 的 `element_index`。序列执行遇到 `isError=true` 的 tool result 后停止。为了避免终端被截图 payload 淹没，CLI pretty-print 时会把 `image.data` 的 base64 摘要成占位文本，但内部 tool result 结构和 MCP 返回仍保持完整。
