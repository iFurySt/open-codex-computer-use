...
这个仓库当前已经从模板收敛成一个本地 `computer-use` 项目。主线仍是 Swift 实现的 macOS automation MCP server，同时新增了实验性的 Windows 和 Linux runtime，用独立 Go 二进制暴露同一组 Computer Use tools（macOS 主线 10 个，Windows / Linux runtime 目前 9 个，平台差异见下文）。
...
- `get_app_state` 优先走真实 AX / 窗口截图；真实 app 必须同时有未最小化的 `AXWindow` 和可匹配的 on-screen `CGWindow`。窗口恢复（unhide / activate / `open -b` / `AXRaise` / `AXMain` / `AXFocused`）会改变用户前台焦点，因此默认关闭：默认只读取当前 Space 上已存在的窗口，目标窗口最小化或不在当前 Space 时直接返回官方风格的 `Apple event error -10005: cgWindowNotFound`，并附上“把窗口移到当前 Space / 取消最小化”的提示，不再把 application 根节点或无截图窗口伪装成可操作状态。需要旧行为时按调用传 `allow_window_recovery=true`，或在进程环境里设置 `OPEN_COMPUTER_USE_ALLOW_WINDOW_RECOVERY=1`（显式 `false` 优先于进程开关）；`click_method=sky_click` 的补拍快照始终只读。当目标是仓库内 fixture app 时，回退到 fixture 导出的合成状态。真实 AX tree 默认在 macOS、Linux、Windows 上最多渲染 1200 个节点、64 层深度；显式 `get_app_state` / `snapshot` 可通过 `max_tree_nodes` / `max_tree_depth` 覆盖预算，action tools 的刷新结果仍使用默认预算。snapshot 文本默认截断到 500 字符；显式 `get_app_state` / `snapshot` 可通过 `text_limit` 正整数或 `"max"` 覆盖，action tools 的刷新结果仍使用 500 字符默认值。对 Electron/WebView 这类深层 UI 会压缩空 `AXGroup` / `AXUnknown` wrapper、过滤 `AXScrollToVisible` 噪音和空字符串属性，避免 action-critical 的输入框被无语义容器挤出节点预算；但通用节点中的 `AXPress` / `AXConfirm` / `AXOpen` 子节点会形成文本摘要边界，避免多个可点击选项被合并成一个 container。这类动作节点如果 frame 有效、尺寸紧凑且不包含带 URL 的 `AXLink` 后代，会保留为带窗口相对 `Frame` 的 `button`，并用短文本后代作为按钮摘要，让 icon-only 和文字 Web 控件都能获得可区分的 `element_index`；包含带 URL 的 `AXLink` 后代时保留通用 wrapper 和链接子节点，避免导航链接被摘要吞掉。对原生 open panel / Finder column view 这类把内容放在 `AXContents` / `AXVisibleChildren` 里的控件，也会把可见文件项纳入元素树。打开的弹层不再把页面挤掉：快照除焦点窗口与菜单栏外，还会从应用根收集未被渲染的临时子树（`AXPopover` / `AXSheet` / `AXMenu` / `AXListBox` / `AXDialog`，以及 floating / dialog / system dialog / system floating 子角色的 `AXWindow`；排除菜单栏与已最小化窗口），命中选择策略时用同一套上下文与延续的索引渲染，并追加在 `--- popup ---` 标记之后，因此一次读取同时拿到窗口内容与弹层选项；没有任何打开的弹层时选择为空、输出与旧版逐字节一致。若弹层把背景从无障碍树里整体隐藏（Chromium 弹层组件对 portal 之外内容 `aria-hidden` 时的实测形态：`AXWebArea` 只剩一个打开中的弹层且焦点在弹层内），快照会在末尾追加 `--- popup note ---`，说明背景元素索引暂时不可用、先处理弹层、随后用 `selector` 继续动作，而不会让上层把“页面消失”误判成需要反复重读。
- MCP `tools/list` 的 description / input schema 当前按官方 `computer-use` 的 10 个 tools 文案和参数面收敛，尽量减少 host 侧提示词和 tool surface 偏差。
...
- `click` / `set_value` / `select_text` 在动作前会把目标元素滚入视口：先对元素本身做 `AXScrollToVisible`，失败则沿祖先链最多上溯 6 层找暴露该动作的祖先，仍失败则对最近的 `AXScrollArea` 祖先按元素相对视口的位置发送 `AXScroll<Up|Down|Left|Right>ByPage`（最多 8 页、每轮读回 frame，frame 不再变化即停止）。越界判定只看“元素 frame 中心点是否落在窗口可视矩形内”，`localFrame` / `windowBounds` 缺失或退化时一律不滚动。滚动后会按同一个 `element_index` 重新解析 AX 句柄并读回 frame 校验；读不回或仍越界时 fail closed，提示重新 `get_app_state`。该能力默认开启，`OPEN_COMPUTER_USE_SCROLL_TARGET_INTO_VIEW=0/false/no/off` 可整条关闭。
- `click` / `set_value` 新增可选 `selector`，与 `element_index` 并存但二选一（同时传直接报错），形如 `button[name=检查变更]`、`combobox[name=类型]`、`[name=返回]`、`[role=textbox][name=用途]` 或裸名字。选择器路径在**动作执行的瞬间**由 OCU 自己重新渲染一次树再解析（role 匹配 AX 角色、去 `AX` 写法、常见别名 family 与快照中显示的本地化 role 文本；name 依次匹配 title / description / value / identifier / placeholder，先精确匹配、再唯一前缀匹配），上层因此不再需要 read → act → read。Chromium 会把同一段文本同时上报成 wrapper 与文本叶子，匹配时按渲染出的父子链把“同一目标的多次上报”折叠成最外层节点；仍然命中多个不相关元素、完全命中不到或选择器语法非法时一律 fail closed：前者列出全部候选，后者列出角色内最接近的候选，不猜第一个。`element_index` 语义不变（仍只对当次快照有效），仍使用缓存快照；选择器动作会多付一次树渲染。
- `select_text` 以元素的 `kAXValueAttribute` 作为可搜索文本，在 `kAXSelectedTextRangeAttribute` 上按 UTF-16 偏移落地选区（`CFRange` 的语义单位，不是 Swift `Character` 计数）；`text` 命中不唯一时 fail closed，返回要求用 `prefix` / `suffix` 消歧的错误，而不是静默选第一个匹配。`selection` 支持 `text` / `cursor_before` / `cursor_after`，设置完成后会读回真实 range：如果目标 app 没有接受这次选区（例如部分 Web 输入控件），result 里会附一条 verification note，不把未生效的选区伪装成成功。
- `click` / `set_value` 在执行真实动作前后，会额外驱动一层透明 `SoftwareCursorOverlay` window：两者的移动阶段现在共用一条 heading-driven 的官方风格 motion 内核，显式把“当前 cursor 朝向”和“最终 resting pose”一起喂给选路器，优先生成需要时先掉头、再沿车头方向推进的 C 形/单侧大弧轨迹；首次显示时按官方 binary 的 fresh state 从 AppKit 全局 `(0,0)` window origin 生成起点，后续动作继续复用上一帧 visible tip。真正显示出来的 cursor 不再直接等于 path sample，而是经过一层独立的 visual dynamics 状态，把 visible tip、velocity、angle 和 fog/offset 持续推进。`click` 结尾会衔接 click pulse 和更明显但仍然很小的 rotate wobble，`set_value` 则只做 settle / idle，不给 pulse；两者收尾后会在目标点继续保持 idle 状态，等待下一次动作时 tip 保持 anchored、只保留可感知的小角度摆动。cursor 的生命周期只由 turn 边界决定：**没有 idle 隐藏定时器**（旧的“30 秒无动作就 cleanup”已删除，因为一轮 turn 里模型思考超过 30 秒很常见，会让光标在回合中途整支消失），只有宿主发出 `turn-ended`、显式 reset 或 `OPEN_COMPUTER_USE_VISUAL_CURSOR=0` 才会消失并清掉本轮位置状态。panel 层级也不再跟随目标窗口 layer 落到 `.normal`：普通窗口（layer 0）下 cursor panel 固定 `.floating`(3)。原因是 OCU 是永不 active 的 accessory app，`.normal` 层级属于非活跃窗口组，任何一次前台切换都会让活跃 app 的窗口盖住光标，而且 `order(.above, relativeTo:)` 绑定的外部窗口一旦被系统 orderOut，光标会跟着一起消失；只有目标窗口本身在 `.floating` 及以上（原生菜单 / popover / panel）时才继续相对该窗口排序。`NSWorkspace.didActivateApplicationNotification` 会触发一次无条件“重新置前”，只重排、不写 frame、不改 level，因此不会重新引入 idle 抖动。整个窗口交互面被收进 `CursorOverlayPanelHosting` / `CursorOverlayEnvironment` 注入缝，`CursorOverlayVisibilityTests` 用假 panel host 直接断言“目标窗口消失 / 别的 app 抢到前台后，光标仍 visible 且 frame 不变”。连续动作的 travel 现在还会去抖：目标与上次落点相差 ≤2pt 时只保留光标、不重复移动与 settle；目标变化但距上次动画不足合并窗口（默认 400ms，可用 `OPEN_COMPUTER_USE_VISUAL_CURSOR_COALESCE_MS` 调成任意非负毫秒值，`0` 恢复逐次动画；非法值回落到 400ms）时直接把 cursor 落到新目标、不播 Bezier 也不播 click pulse，因此一轮连续动作在观感上最多每 400ms 播一次移动动画；合并窗口锚定“上一次真正播动画”的时刻，同目标时既不移动也不重新锚定，避免亚像素漂移把 cursor 慢慢带走。空闲期现在是**绝对静止**的：每个显式动作（move / reposition / settle / pulse）都会刷新 `lastInteractionAt`，idle 的 60 Hz tick 只在该时刻之后的 1 秒节拍窗口内运行（`OPEN_COMPUTER_USE_VISUAL_CURSOR_IDLE_SWAY_MS` 可调，`0` 表示动作落定即冻结；保留 1 秒是因为 idle smoke 仍要求“tip 锚定 + 可见的微小转角变化”），窗口结束由 `CursorIdleDriver` 自己 `invalidate()` 定时器，之后 tick 直接 return；`startIdleAnimation` 先停旧定时器，因此 `settle` + `pulseClick` 不会再各留一个 60 Hz writer（此前多个定时器同时推进共享 `idlePhase`，是“小箭头原地抖”的直接来源之一）。所有 panel 写入统一过 `CursorPanelWriteGate`：frame origin 先取整到整点、与上次写入相同就整帧跳过（不再把弹簧的亚像素尾巴喂给 window server），`level` 相同不写，`order(.above, relativeTo:)` 只在显示/隐藏或目标窗口变化时执行——idle tick 不再每帧重排 panel。`OPEN_COMPUTER_USE_VISUAL_CURSOR_DEBUG_STATS=1` 时，cursor 隐藏 / reset 会向 stderr 打一行累计计数（frameWrites / skippedFrameWrites / levelWrites / reorders / skippedReorders / idleTicks / suppressedIdleTicks / invalidatedIdleTimers）。

- 目标窗口的 frame 不再是“快照时刻”的常量：`ComputerUseService.currentSnapshot` 在每次动作前用 `CGWindowListCopyWindowInfo(.optionIncludingWindow)` 重读目标窗口的实时几何（同一窗口、同一尺寸、仅移动 → 只 patch `windowBounds`，尺寸变化或换了窗口 → 整份重取），因此用户在两次动作之间把窗口拖到另一块屏后，软件光标、坐标点击与截图映射都不会再沿用旧屏坐标。overlay 侧保留 `CursorRestingAnchor`（窗口局部偏移 + 当时的 screen-state frame），窗口移动/换屏后由此重新推导 tip；`kAXWindowMovedNotification` / `kAXWindowResizedNotification` 上挂了一只只读 `AXObserver`（`OPEN_COMPUTER_USE_WINDOW_MOVE_WATCH=0` 关闭）做事件式跟随，动作前还会兜底校验“光标所在屏 == 目标窗口所在屏”，不一致就重算重画且不隐藏光标。

- macOS 的可视化对齐官方 binary：只有 `SoftwareCursorOverlay`（雾状光晕 + Bezier 移动 + click pulse），**没有"目标高亮环"**——官方 binary 里没有这个概念，OCU 自建的 `TargetHighlightOverlay` / `TargetHighlightLifetime` / `TargetHighlightStyle` 以及 `OPEN_COMPUTER_USE_TARGET_HIGHLIGHT_STYLE` / `OPEN_COMPUTER_USE_DEBUG_HIGHLIGHT` 开关已整体删除，不要再加回来。`click` / `set_value` / `select_text` / `perform_secondary_action` / `scroll` 共用 `VisualInteractionChoreographer` 的 advisory 顺序：光标先移动到目标并停留约 120ms 的到达节拍，然后才执行真实动作；`perform_secondary_action` / `scroll` 也因此补上了移动阶段。到达后由调用方按去抖结果（`approach.playsClickPulse`）决定是否由 `SoftwareCursorOverlay.pulseClick` 播点击脉冲；`turn-ended` / `reset` / `OPEN_COMPUTER_USE_VISUAL_CURSOR=0` 负责收起光标。自证入口是 `open-computer-use debug-cursor [--seconds N] [--display N]`：只画光标与 click pulse，单屏（1-based，默认主屏，上限 60s，`defer` 确定性收尾），并打印可直接喂给 `screencapture -R` 的 screen-state 矩形。
...
- 这 9 个 tool 的协议面与 macOS 主线保持一致：`list_apps`、`get_app_state`、`click`、`perform_secondary_action`、`scroll`、`drag`、`type_text`、`press_key`、`set_value`（macOS 主线的 `select_text` 尚未在 Windows runtime 实现）。其中 element-targeted action 会优先复用上一轮 `get_app_state` 的 runtime id / automation metadata，coordinate action 使用 screenshot/window-relative 坐标。
...
- 这 9 个 tool 的协议面与 macOS / Windows 保持一致：`list_apps`、`get_app_state`、`click`、`perform_secondary_action`、`scroll`、`drag`、`type_text`、`press_key`、`set_value`（macOS 主线的 `select_text` 尚未在 Linux runtime 实现）。其中 element-targeted action 会优先复用上一轮 `get_app_state` 的 runtime path metadata，coordinate action 使用 screenshot/window-relative 坐标。
