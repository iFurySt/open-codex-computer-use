...
- 在 visual cursor 之外，`click` / `set_value` / `select_text` / `perform_secondary_action` / `scroll` 还会在动作前用 `TargetHighlightOverlay` 给目标元素画一个高亮环，默认停留 450ms 后 350ms 淡出。它复用同一套 screen-state → AppKit 全局坐标换算与无边框透明 `NSPanel`，强制 `canBecomeKey/Main = false`、`ignoresMouseEvents = true`，`localFrame` 为空或与窗口可视矩形不相交时完全不显示，并且跟随 `OPEN_COMPUTER_USE_VISUAL_CURSOR` 与 `turn-ended` / `reset` 一起清理。
