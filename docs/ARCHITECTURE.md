...
- macOS 在这 9 个之外额外提供 `js` / `js_reset`：`js` 用一个常驻 JavaScriptCore runtime 把这些 action 暴露成同步的 `cua` API，让模型在一次调用里写完「取状态 → 找元素 → 操作 → 校验 → 重试」的整段流程，而不是一步一次 tool call。原有 9 个 tool 的语义和参数面完全不变，`js` 只是它们之上的一层组合入口；`OpenComputerUseJavaScriptShim` 提供执行时限所需的 JavaScriptCore C 接口。API 同步是刻意的：所有 action 都在进程内执行，不需要 promise 或 top-level await。
