...
- macOS 在这 9 个之外额外提供 `run_intent`：按 `<bundle id>.<intent name>` 直接执行应用自己的 App Intent，而不是去驱动它的界面。macOS 没有可用的免 entitlement API 直接触发 App Intent——私有 LinkServices executor 会拒绝任何未经校验的调用方——唯一受支持的路径是 Shortcuts，因此实现会生成一个只包含该 action 的 workflow，用 `shortcuts sign` 签名后打开，由用户确认添加一次；之后每次调用都走 `shortcuts run`。`shortcuts list` 就是已添加内容的记录，不再单独维护一张会和它漂移的绑定表。
