...
- `ComputerUseService` 负责把 Computer Use tool 请求映射到本地能力，`ComputerUseToolDispatcher` 则把这些 tool 的参数解析与 service 方法分发收敛成 MCP server 和 `open-computer-use call` 共用的一层。
...
- macOS 在这 9 个之外额外提供 `query`：按 text / role 在目标窗口里做定向 AX 查找，不渲染树也不截图，返回的 `index` 可以直接交给既有 element action。它是对官方 tool surface 的一处有意偏离，换来的是“找一个控件”不必付整棵树加一张截图的代价；官方 9 个 tool 的语义和参数面不受影响。`query` 分配的 index 从 `1_000_000` 起单调递增，永不复用，因此后续快照不会把同一个 index 指到另一个控件上。
