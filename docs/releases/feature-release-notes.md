...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-09-15 | macOS App Intent 执行 | 应用把某个操作暴露成 App Intent 时，可以直接按身份执行，不需要窗口可见或在前台，也不必用界面把流程点一遍。 | macOS 新增 `run_intent` tool（`bundle_id` / `action_id` / `input` / `parameters`）：经 Shortcuts 执行应用自有 App Intent，首次调用生成并打开签名后的单动作 shortcut 供用户确认添加一次；拒绝 `is.workflow.` 内置动作，identifier 做严格校验，子进程 stdin 接 `/dev/null` 并有 120s 上限。 |
