...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-09-15 | macOS js 组合调用 | 多步流程可以在一次 tool call 里写完，不必每个动作往返一次；循环、重试和条件判断直接写在代码里。 | macOS 新增 `js` / `js_reset` tool：常驻 JavaScriptCore runtime 把既有 action 暴露成同步 `cua` API，`write(...)` 产出文本、`emitImage(...)` 产出图片，每次调用有独立函数作用域，跨调用状态挂在 `globalThis` 上；原有 9 个 tool 不变。 |
