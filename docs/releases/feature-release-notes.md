...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-09-19 | CLI 参数容错与截图瘦身 | Agent 调用 CLI 时常见参数别名（如 `index`、`button`）自动映射，杜绝报错；Base64 截图解耦输出到本地文件，避免终端截断和 Token 爆炸。 | launcher 新增参数别名标准化中间件（`index -> element_index`、`button -> mouse_button`、`amount -> pages`、`start/end -> from/to`）；默认将大图转存至 `/tmp/ocu_last_screenshot.png` 并在 stdout 返回轻量引用（支持 `--raw-image`）；支持通过 `text` / `title` 触发带角色权重的语义化智能点击；更新 SKILL 及 usage 参考文档。 |
