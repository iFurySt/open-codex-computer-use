...
| 日期 | 功能域 | 用户价值 | 变更摘要 |
| --- | --- | --- | --- |
| 2026-09-15 | screenshot 编码 | 截图返回更快：同样的画面字节数显著下降，模型看到的像素尺寸不变，复杂页面不再因为图片过大而变慢。 | `get_app_state` 等工具返回的窗口图片改为 JPEG（quality 0.8，最长边 1280），移除 900 KB 字节上限及其逐步缩小循环；`ToolResultContentItem.pngImage` 更名为 `jpegImage`，`AccessibilitySnapshot.screenshotPNGData` 更名为 `screenshotData`。 |
