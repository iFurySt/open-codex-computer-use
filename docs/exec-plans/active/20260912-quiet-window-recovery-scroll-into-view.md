...
- 2026-09-12：P11 坐标陈旧与抢焦点**不同源**。坐标陈旧的可复现链路是缓存：`ComputerUseService.currentSnapshot`（`ComputerUseService.swift:1159-1165`）把 `snapshotsByApp` 里的快照直接交给动作，`windowBounds` / `targetWindowID` 因此停在 `get_app_state` 时刻；`windowPointToGlobalPoint`（`2205-2211`）与 `makeVisualCursorTarget`（`184-220`）仍按旧 frame 算点，动作收尾的 `refreshSnapshot` 才更新缓存，所以现象是“先留在旧屏、稍后又正常”。扫过全部会改变前台焦点的 API（`unhide` / `activate` / `open -b` / `AXRaise` / `AXMain` / `AXFocused`）后确认它们只出现在 `AccessibilitySnapshot.recoverVisibleWindow`（`AccessibilitySnapshot.swift:300-324`，`.allowActivation` 才可达）、`activateClickTarget`（`ComputerUseService.swift:1436-1452`，由 `activationOnlyClickFallbackAllowed` 门控）与 `InputSimulation.prepareAppForGlobalPointerInput`（`InputSimulation.swift:48-56`，仅 global 指针路径），默认路径零激活；本次不新增任何激活 API，新增的 `AXObserver` 只注册 `AXWindowMoved` / `AXWindowResized` 两个只读通知（`CursorCrossScreenTests.testTheMoveWatchOnlyEverRegistersReadOnlyNotifications` 钉住）。
