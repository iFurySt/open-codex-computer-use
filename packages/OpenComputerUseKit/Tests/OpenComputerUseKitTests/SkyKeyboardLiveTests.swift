...
@MainActor
final class SkyKeyboardLiveTests: XCTestCase {
    ...
    func testCoveredChromeReceivesSkyKeyTypingWithoutForegroundSideEffects() throws {
        guard ProcessInfo.processInfo.environment["OPEN_COMPUTER_USE_RUN_SKY_KEY_LIVE_TEST"] == "1" else {
            throw XCTSkip("Set OPEN_COMPUTER_USE_RUN_SKY_KEY_LIVE_TEST=1 to run the isolated Chrome live test")
        }
        ...
        let testRoot = Self.packageRoot.appendingPathComponent(".build", isDirectory: true)
            .appendingPathComponent("ocu-sky-key-live-\(UUID().uuidString)", isDirectory: true)
        ...
        defer {
            do {
                ...
                print("sky_key live test cleanup warning: \(error)")
            }
        }
        ...
        let readyWindow = try waitForWindow(
            ...
            nameContaining: "ocu-sky-key-ready"
        )
        print("sky_key live test: target window=\(readyWindow.id) owner pid=\(readyWindow.pid)")
        stop(coverProbe)
        ...
        XCTAssertTrue(
            ...
            "The controlled Chrome window must be fully covered before sky_key"
        )
        ...
        let target = SkyKeyboardTarget(windowID: coveredWindow.id, pid: coveredWindow.pid)

        try SkyKeyboardDispatcher.typeText(target: target, text: "hello", spi: spi)
        let typedWindow = try waitForWindow(pid: readyWindow.pid, nameContaining: "ocu-sky-key-value-hello|")
        XCTAssertTrue(
            typedWindow.name.hasPrefix("ocu-sky-key-value-hello|"),
            "sky_key typing must land in the covered Chrome input exactly once"
        )

        try SkyKeyboardDispatcher.pressKey(target: target, key: "backspace", spi: spi)
        try SkyKeyboardDispatcher.pressKey(target: target, key: "shift+a", spi: spi)
        _ = try waitForWindow(pid: readyWindow.pid, nameContaining: "ocu-sky-key-value-hellA|")

        // Menu-bar key equivalents go through the target's Edit menu.
        try SkyKeyboardDispatcher.pressKey(target: target, key: "cmd+a", spi: spi)
        try SkyKeyboardDispatcher.typeText(target: target, text: "Ok!", spi: spi)
        let editedWindow = try waitForWindow(pid: readyWindow.pid, nameContaining: "ocu-sky-key-value-Ok!|")
        RunLoop.current.run(until: Date().addingTimeInterval(0.5))
        let finalWindow = try XCTUnwrap(windows().first(where: { $0.id == editedWindow.id }))
        XCTAssertEqual(
            finalWindow.name,
            "ocu-sky-key-value-Ok!|unfocused",
            "sky_key must deliver keys, key-binding chords and menu chords, then release Chrome's synthetic key state"
        )

        // Control: the plain CGEvent.postToPid keyboard path that `auto` uses.
        // Chromium receives the key events but refuses text insertion while its
        // window is not key. Observed, not asserted, so a future Chromium change
        // cannot fail the sky_key regression.
        try InputSimulation.typeText("plain", pid: coveredWindow.pid)
        RunLoop.current.run(until: Date().addingTimeInterval(0.5))
        let controlTitle = windows().first(where: { $0.id == editedWindow.id })?.name ?? ""
        print("sky_key live test: public postToPid control title after release=\(controlTitle)")

        XCTAssertEqual(
            NSWorkspace.shared.frontmostApplication?.processIdentifier,
            frontPIDBefore,
            "sky_key must not change the frontmost app"
        )
        let foregroundStateAfter = try waitForFixtureState(
            pid: cover.processIdentifier,
            failure: "The foreground fixture state was unavailable after sky_key"
        ) { _ in true }
        XCTAssertEqual(foregroundStateAfter.isActive, true, "sky_key must keep the foreground app active")
        XCTAssertEqual(foregroundStateAfter.isKeyWindow, true, "sky_key must keep the foreground window key")
        XCTAssertEqual(
            foregroundStateAfter.focusedIdentifier,
            foregroundStateBefore.focusedIdentifier,
            "sky_key must preserve the foreground first responder"
        )
        XCTAssertEqual(
            foregroundStateAfter.activationLossCount,
            foregroundStateBefore.activationLossCount,
            "sky_key must not transiently resign the foreground app"
        )
        XCTAssertEqual(
            foregroundStateAfter.keyWindowLossCount,
            foregroundStateBefore.keyWindowLossCount,
            "sky_key must not transiently resign the foreground key window"
        )
        XCTAssertEqual(
            foregroundStateAfter.elements.first(where: { $0.identifier == "fixture-input" })?.value,
            foregroundStateBefore.elements.first(where: { $0.identifier == "fixture-input" })?.value,
            "sky_key keystrokes must not leak into the foreground text field"
        )
        ...
        guard
            ...
            let targetIndexAfter = orderedAfter.firstIndex(where: { $0.id == editedWindow.id })
        else {
            return XCTFail("Could not verify final window z-order")
        }
        XCTAssertLessThan(coverIndexAfter, targetIndexAfter, "sky_key must not raise the Chrome window")
    }
    ...
    <!doctype html>
    <meta charset="utf-8">
    <title>ocu-sky-key-ready</title>
    <style>
      html, body { width: 100%; height: 100%; margin: 0; }
      input { width: 90%; font: 24px system-ui; margin: 24px; }
    </style>
    <input id="target" autofocus placeholder="sky_key probe">
    <script>
      const input = document.querySelector('#target');
      input.focus();
      const update = () => {
        document.title = `ocu-sky-key-value-${input.value}|${document.hasFocus() ? 'focused' : 'unfocused'}`;
      };
